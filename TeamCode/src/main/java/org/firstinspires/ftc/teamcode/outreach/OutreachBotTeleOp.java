...
import org.firstinspires.ftc.teamcode.outreach.utilities.ImprovedGamepad;


@SuppressWarnings("unused")
@TeleOp(name = "OutreachBotV1", group = "OutreachBot")
public class OutreachBotTeleOp extends OpMode {
    ...
    public void loop() {
        ...
        rightPower = (gamepadInControl.right_stick.y.getValue() / OutreachBotHardware.STRAIGHT_POWER_WEIGHT) - (gamepadInControl.left_stick.x.getValue() / OutreachBotHardware.TURN_WEIGHT);
        leftPower = (gamepadInControl.right_stick.y.getValue() / OutreachBotHardware.STRAIGHT_POWER_WEIGHT)  + (gamepadInControl.left_stick.x.getValue() / OutreachBotHardware.TURN_WEIGHT);
        robot.leftDrive.setPower(leftPower);
        robot.rightDrive.setPower(rightPower);

        if (gamepadInControl.left_bumper.isPressed() || gamepadInControl.left_trigger.isPressed()){
            robot.claw.setPosition(OutreachBotHardware.CLAW_OPEN_POSITION);
        }

        if (gamepadInControl.right_bumper.isPressed() || gamepadInControl.right_trigger.isPressed()){
            robot.claw.setPosition(OutreachBotHardware.CLAW_CLOSED_POSITION);
        }
